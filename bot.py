...
SERIES_TICKER = getenv_first(["SERIES", "KALSHI_SERIES", "KALSHI_SERIES_TICKER"], "KXETH1H")
...
SIGMA_FLOOR = env_float("SIGMA_FLOOR", 0.20)
SIGMA_CEIL = env_float("SIGMA_CEIL", 1.50)
...
def _eth_is_safe(side: str, spot: float, lo: Optional[float], hi: Optional[float],
    ...
    """
    buffer = DUMP_ETH_SAFE_BUFFER_LATE if secs_to_close < DUMP_ETH_SAFE_CUTOFF_SECONDS else DUMP_ETH_SAFE_BUFFER_EARLY
...
def should_dump_position(
    ...
    # Was 60s — ETH can move $3 in 60s (σ=0.40, √60=7.7). 30s is $2.20, safer.
    ...
    if st.entry_time > 0 and secs_to_close <= HOLD_TO_SETTLE_SECONDS:
        ...
        if not eth_safe_for_hold or eth_hold_dist < HOLD_ETH_DANGER_BUFFER:
            # ETH is near the boundary — DON'T suppress dumps, let normal logic decide
            log.warning(
                f"[HOLD OVERRIDE] ETH near boundary (dist=${eth_hold_dist:.0f} < ${HOLD_ETH_DANGER_BUFFER:.0f}) "
            ...
            return False, f"late_entry_hold_to_settle_t={secs_to_close}s_dist=${eth_hold_dist:.0f}"
    ...
    if rapid_drop >= DUMP_RAPID_DROP_THRESHOLD:
        log.warning(
            f"[BAIL RAPID DROP] ETH NOT safe (dist=${eth_distance:.0f}) AND "
    ...
    if st.entry_price_cents is not None:
        ...
        if unrealized_loss_per_contract >= DUMP_MAX_LOSS_CENTS_PER_CONTRACT:
            log.warning(
                f"[BAIL HARD STOP] ETH NOT safe (dist=${eth_distance:.0f}) AND "
    ...
    if DUMP_ON_PROB_REVERSAL and DUMP_EARLY_EXIT_ENABLED:
        ...
        if drop_from_peak >= effective_threshold:
            ...
            log.warning(
                f"[BAIL REVERSAL] ETH NOT safe (dist=${eth_distance:.0f}) AND "
    ...
    if current_prob < DUMP_PROB_FLIP:
        log.warning(
            f"[BAIL FLOOR] ETH NOT safe (dist=${eth_distance:.0f}) AND "
    ...
    return False, (
        ...
        f"_drop={drop_from_peak:.0%}_rdrop={rapid_drop:.0%}_eth_dist=${eth_distance:.0f}"
    )
...
def evaluate_scalp(
    ...
    # At σ=0.40, t=60: $4.65.  t=30: $3.29.  t=15: $2.32.  t=10: $1.90.
...
def main() -> None:
    ...
    log.warning(
        ...
        f"eth_buffer_early=${DUMP_ETH_SAFE_BUFFER_EARLY:.0f} eth_buffer_late=${DUMP_ETH_SAFE_BUFFER_LATE:.0f}"
    )
    log.warning(
        ...
        if fast_lane:
            ...
            if REQUIRE_TREND_ALIGNMENT and prob_trend_ok:
                ...
                if chosen_side == "yes" and "down" in spot_dir and abs(spot_move) > 1.50:
                    misaligned = True
                    mismatch_reason = f"prob_yes but ETH 60m={spot_dir}(${spot_move:+.0f})"
                elif chosen_side == "no" and "up" in spot_dir and abs(spot_move) > 1.50:
                    misaligned = True
                    mismatch_reason = f"prob_no but ETH 60m={spot_dir}(${spot_move:+.0f})"
                elif chosen_side == "yes" and "down" in spot_short_dir and abs(spot_short_move) > 1.50:
                    misaligned = True
                    mismatch_reason = f"prob_yes but ETH 30m={spot_short_dir}(${spot_short_move:+.0f})"
                elif chosen_side == "no" and "up" in spot_short_dir and abs(spot_short_move) > 1.50:
                    misaligned = True
                    mismatch_reason = f"prob_no but ETH 30m={spot_short_dir}(${spot_short_move:+.0f})"
