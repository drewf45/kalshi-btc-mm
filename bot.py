...
SCALP_DISTANCE_TIERS = [
    (400.0, 0.30),   # $400+ from strike: safe but still capped
    (200.0, 0.20),   # $200-400: moderate
    (100.0, 0.10),   # $100-200: small — one bad scalp undoes 30 good ones
    (50.0,  0.05),   # $50-100: minimal — not worth the tail risk
]
SCALP_MAX_ENTRY_PRICE = 97        # Max 97¢ — need at least 3¢/contract profit to justify risk
SCALP_MIN_PROB = 0.96             # Must be near-certain for 99¢ entries — 80% @ 99¢ = -19¢/ct EV
...
# When BTC moves more than $X in the last N seconds, the probability estimate
...
WHIPSAW_THRESHOLD_USD = 150.0      # Skip if BTC moved > $150 in the window
...
def main() -> None:
    ...
    while True:
        ...
        # WHIPSAW FILTER: if BTC moved > $150 in the last 45s, the probability
        ...
        if is_whipsaw:
            if (now - last_state_log) >= LOG_STATE_EVERY_SECONDS:
                log.warning(
                    f"[WHIPSAW] SKIPPING entry — BTC moved ${whipsaw_move:.0f} in "
