...
# Asset-specific book premium — SOL has medium spreads
POSTER_BOOK_PREMIUM = 2  # SOL — medium spreads

# SOL Zone win rates from clean-entry markets
# Note: SOL NO 51-65¢ was a true negative (33% WR) — blocked
# Zones 21-40¢ and 66-80¢ have no clean data — blocked
SOL_ZONE_WIN_RATES = [
    (1,  10,  0.929),  # 92.9% WR — 85 markets
    (11, 20,  0.882),  # 88.2% WR — 51 markets
    (21, 40,  0.0),    # No clean data — blocked
    (41, 50,  0.833),  # 83.3% WR — 6 markets (small sample)
    (51, 65,  0.0),    # 33% WR — true negative, blocked
    (66, 80,  0.0),    # No clean data — blocked
    (81, 90,  0.900),  # 90.0% WR — 10 markets
    (91, 99,  0.950),  # 95.0% WR — 40 markets
]

SOL_SCORE_THRESHOLD = 2.5

def get_zone_win_rate(book_price: int) -> float:
    for (lo, hi, wr) in SOL_ZONE_WIN_RATES:
        if lo <= book_price <= hi:
            return wr
    return 0.0
...
def evaluate_poster_entry(p_yes: float, p_no: float,
    ...
    if yes_ask is not None:
        ...
        log.info(
            ...
            f"score={score:.2f} threshold={SOL_SCORE_THRESHOLD}"
        )

        if gap > 0 and score >= SOL_SCORE_THRESHOLD:
            post_price = yes_ask + POSTER_BOOK_PREMIUM
            post_price = max(1, min(99, post_price))
            log.warning(
                f"[TRIGGER-YES] score={score:.2f} >= {SOL_SCORE_THRESHOLD} "
                f"post={post_price}¢"
            )
    ...
    if no_ask is not None:
        ...
        if gap > 0 and score >= SOL_SCORE_THRESHOLD:
            ...
            post_price = max(1, min(99, post_price))
            log.warning(
                f"[TRIGGER-NO] score={score:.2f} >= {SOL_SCORE_THRESHOLD} "
                f"post={post_price}¢"
            )
    ...
    while True:
        ...
        if (now - last_amend) >= POSTER_AMEND_INTERVAL:
            try:
                ...
                if gap <= 0 or score < SOL_SCORE_THRESHOLD:
                    log.warning(
                        f"[GAP-AMEND] Score dropped "
                        f"({score:.2f} < {SOL_SCORE_THRESHOLD}) "
...
def main() -> None:
    ...
    log.warning(f"[IRON RULES] RULE 3: Score >= {SOL_SCORE_THRESHOLD} to post | cost cap ${MAX_COST_PER_MARKET}")
    ...
    log.warning(
        ...
        f"SCORE_THRESHOLD={SOL_SCORE_THRESHOLD} "
            ...
            if entry is None:
                ...
                if (now - last_state_log) >= LOG_STATE_EVERY_SECONDS:
                    ...
                    log.info(
                        ...
                        f"threshold={SOL_SCORE_THRESHOLD} "
...
if __name__ == "__main__":
    try:
        ...
        log.exception(f"FATAL: {e} | {ASSET} V7 SCORE={SOL_SCORE_THRESHOLD} ALLOC=${BOT_ALLOCATION}")
        sys.exit(1)
