...
log = logging.getLogger("kalshi-sol-bot")
...
SERIES_TICKER = getenv_first(["SERIES", "KALSHI_SERIES", "KALSHI_SERIES_TICKER"], "KXSOL15M")
...
# from other bots sharing the same Kalshi account.
BOT_ID = "SOL"
...
REQUIRE_TREND_ALIGNMENT = False    # DISABLED — was True (trend data kept, just not gating on it)
# HIGH-CERTAINTY FAST LANE: if prob is this high, skip trend/momentum checks entirely
# Rationale: 90%+ prob means SOL is well inside the range. The outcome is decisive.
...
MAX_CONTRACTS = 3            # Hard cap — SOL should never hold more than 3 contracts.
...
POST_COOLDOWN_EDGE_BOOST = 0.0  # DISABLED — was 0.02 (extra edge after loss streak)
...
DAYTIME_SIZE_MULTIPLIER = 1.0    # disabled: was 1.25 — time-of-day sizing is now a no-op
OVERNIGHT_SIZE_MULTIPLIER = 1.0  # disabled: was 0.75 — time-of-day sizing is now a no-op
...
SESSION_DRAWDOWN_PAUSE_SECONDS = 1800    # Pause 30 min (matches spec)
...
# -------------- TREND TRACKING (know what SOL is doing) --------------
...
# -----------------------------
def fetch_sol_spot_usd(session: requests.Session, timeout: float = 5.0) -> Optional[float]:
    try:
...
def wait_for_fill(client: KalshiClient, order_id: str, market: str) -> Tuple[str, int]:
    ...
    for attempt in range(1, FILL_CHECK_RETRIES + 1):
        ...
        if order is None:
            ...
            # Return unknown so the caller can handle it with internal state.
...
# =====================================================
# INTERNAL POSITION TRACKER — multi-bot isolation
# =====================================================
# Each bot tracks its own positions internally instead of reading
# account-wide positions from the Kalshi API. This prevents one bot
# from seeing another bot's positions and entering false HOLD states.

@dataclass
class TrackedPosition:
    """A position this bot placed and got filled."""
    market_id: str
    side: str           # "yes" or "no"
    qty: int
    entry_price_cents: int
...
class PositionTracker:
    """Track this bot's own positions independently of Kalshi API positions."""

    def __init__(self, bot_id: str):
        self.bot_id = bot_id
        self._positions: Dict[str, TrackedPosition] = {}

    def record_fill(self, market_id: str, side: str, qty: int,
                    entry_price_cents: int, order_id: str) -> None:
        ...
        log.warning(
            f"[POS TRACK] {self.bot_id} RECORDED fill: {market_id} "
            f"{side.upper()} qty={qty} entry={entry_price_cents}¢ oid={order_id}"
        )

    def add_qty(self, market_id: str, additional_qty: int) -> None:
        """Add contracts to existing position (e.g., scalp fills)."""
        if market_id in self._positions:
            self._positions[market_id].qty += additional_qty
            log.info(f"[POS TRACK] {self.bot_id} ADDED {additional_qty} contracts to {market_id} "
                     f"(total={self._positions[market_id].qty})")

    def update_qty(self, market_id: str, new_qty: int) -> None:
        """Update qty for a market (e.g., partial fill reconciliation)."""
        if market_id in self._positions:
            old_qty = self._positions[market_id].qty
            self._positions[market_id].qty = new_qty
            log.info(f"[POS TRACK] {self.bot_id} UPDATED qty {old_qty} -> {new_qty} for {market_id}")

    def record_exit(self, market_id: str) -> None:
        """Clear position after dump sell or market settlement."""
        removed = self._positions.pop(market_id, None)
        if removed:
            log.warning(f"[POS TRACK] {self.bot_id} CLEARED position: {market_id} "
                        f"(was {removed.side.upper()} qty={removed.qty})")

    def get_signed_qty(self, market_id: str) -> int:
        """Get signed position qty: positive=YES, negative=NO, 0=none."""
        p = self._positions.get(market_id)
        if p is None or p.qty <= 0:
            return 0
        return p.qty if p.side == "yes" else -p.qty

    def has_position(self, market_id: str) -> bool:
        """Check if we have an active position in this market."""
        p = self._positions.get(market_id)
        return p is not None and p.qty > 0

    def get_qty(self, market_id: str) -> int:
        """Get unsigned qty for a market."""
        p = self._positions.get(market_id)
        return p.qty if p is not None else 0

    def committed_usd(self) -> float:
        """Total USD committed to all open positions."""
        return sum((p.entry_price_cents * p.qty) / 100.0
                   for p in self._positions.values() if p.qty > 0)

    def make_client_order_id(self) -> str:
        """Generate a tagged client_order_id for order identification."""
...
class SpotTrend:
    """
    Tracks SOL spot price over a rolling window to detect trends.
    ...
    def trade_alignment(self, side: str, lo: Optional[float], hi: Optional[float],
        ...
        Logic:
        - YES bet = we think SOL will stay ABOVE lo (or in range)
        - NO bet = we think SOL will stay BELOW hi (or in range)
        - If SOL is trending UP strongly and we want NO → against trend
        - If SOL is trending DOWN strongly and we want YES → against trend
        """
...
class ProbTrend:
    ...
    def should_buy(self, side: str, current_prob: float, secs_to_close: int = 0) -> Tuple[bool, str]:
        ...
        Time-dependent strictness (more certain early, relax as outcome clarifies):
          EARLY (5-7 min): Require active trend match — SOL still has time to swing.
...
def choose_trade(
    ...
    # SETTLEMENT LOCK: near expiry, relax probability gate but STILL require edge.
    ...
    # FIX: Require at least 1% edge (reduced from normal EDGE_MIN since variance is lower
...
def _sol_is_safe(side: str, spot: float, lo: Optional[float], hi: Optional[float],
                  secs_to_close: int) -> Tuple[bool, float]:
    """
    Check if SOL spot price is safely on our side of the market boundary.
    Returns (is_safe, buffer_distance_usd).

    This is THE key check: if SOL is on our side, the book is lying.
    ...
    """
    buffer = DUMP_SOL_SAFE_BUFFER_LATE if secs_to_close < DUMP_SOL_SAFE_CUTOFF_SECONDS else DUMP_SOL_SAFE_BUFFER_EARLY

    if side == "yes" and lo is not None:
        # YES wins if SOL stays ABOVE lo. Safe if spot > lo + buffer.
        distance = spot - lo
        return distance >= buffer, distance
    elif side == "no" and hi is not None:
        # NO wins if SOL stays BELOW hi (range market). Safe if spot < hi - buffer.
        distance = hi - spot
        return distance >= buffer, distance
    elif side == "no" and lo is not None:
        # NO wins if SOL drops BELOW lo (up-or-down market, no hi).
        # Safe if spot < lo - buffer (SOL is well below the strike).
        distance = lo - spot
        return distance >= buffer, distance
...
def should_dump_position(
    ...
    KEY PRINCIPLE: Before any bail trigger fires, check if SOL is on our side
    of the boundary. If it is, the book is lying — HOLD. Only bail when
    SOL has actually moved against us.

    BANKROLL PROTECTION: Never lose more than 5% of balance or 50% of position
    cost on a single trade. This fires before SOL check — no position justifies
    ...
    # For earlier entries, normal dump logic applies — SOL can still move.
    # Was 60s — SOL can move $1.50 in 60s (σ=0.05, √60=7.7). 30s is $0.27, safer.
    # CRITICAL FIX: Even within hold window, if SOL is near the boundary,
    ...
    HOLD_SOL_DANGER_BUFFER = 0.30  # If SOL is within $0.30 of boundary, DON'T suppress dumps (was $0.18)
    ...
    if st.entry_time > 0 and secs_to_close <= HOLD_TO_SETTLE_SECONDS and not is_scalp_entry:
        ...
        sol_safe_for_hold, sol_hold_dist = _sol_is_safe(st.side, spot, lo, hi, secs_to_close)
        if not sol_safe_for_hold or sol_hold_dist < HOLD_SOL_DANGER_BUFFER:
            # SOL is near the boundary — DON'T suppress dumps, let normal logic decide
            log.warning(
                f"[HOLD OVERRIDE] SOL near boundary (dist=${sol_hold_dist:.2f} < ${HOLD_SOL_DANGER_BUFFER:.2f}) "
                f"with {secs_to_close}s left — allowing dump checks"
            )
            # Fall through to normal dump logic below
        else:
            # SOL is safely on our side — hold to settlement
            ...
            return False, f"late_entry_hold_to_settle_t={secs_to_close}s_dist=${sol_hold_dist:.2f}"
    ...
    # =============================================================
    # === BANKROLL-PROPORTIONAL STOP: fires BEFORE SOL check ===
        ...
        if current_balance_usd > 0:
            ...
            if total_loss_usd > max_loss_balance:
                log.warning(
                    ...
                    f"overrides SOL safety, protecting bankroll"
                )
    ...
    # =============================================================
    # === TWO-TIER DOLLAR CAP: soft (limit sell) then hard (market sell) ===
    ...
    if st.entry_price_cents is not None and st.qty > 0:
        ...
        if total_loss_usd_hc > DUMP_HARD_MAX_LOSS_USD:
            log.warning(
                ...
                f"force exit to cap damage at ${DUMP_HARD_MAX_LOSS_USD:.2f}"
            )
            return True, f"hard_dollar_cap_${total_loss_usd_hc:.2f}>${DUMP_HARD_MAX_LOSS_USD:.2f}"
        if total_loss_usd_hc > DUMP_SOFT_MAX_LOSS_USD:
            log.warning(
                f"[BAIL SOFT DOLLAR CAP] losing ${total_loss_usd_hc:.2f} > "
                f"${DUMP_SOFT_MAX_LOSS_USD:.2f} soft cap — "
                f"{loss_per_ct_hc}¢/ct × {st.qty}ct "
                f"(entry={st.entry_price_cents}¢ est_exit={exit_price_est_hc}¢) — "
                f"limit sell to exit gracefully"
            )
            return True, f"soft_dollar_cap_${total_loss_usd_hc:.2f}>${DUMP_SOFT_MAX_LOSS_USD:.2f}"
    ...
    if st.entry_price_cents is not None:
        ...
        if catastrophic_loss >= DUMP_CATASTROPHIC_LOSS_CENTS:
            log.warning(
                ...
                f"(entry={st.entry_price_cents}¢ est_exit={exit_price_est}¢) — "
                f"overrides SOL safety, capping damage"
            )
            return True, f"catastrophic_{catastrophic_loss}c_per_contract"

    # =============================================================
    # === SOL SAFETY CHECK: THE MASTER OVERRIDE ===
    # If SOL is on our side of the boundary, DO NOT BAIL.
    # The book can spike, the probability can drop on a thin book,
    # but if SOL is $0.20+ on our side with 1 minute left, we WIN.
    # =============================================================
    sol_safe, sol_distance = _sol_is_safe(st.side, spot, lo, hi, secs_to_close)
    if sol_safe:
        # SOL is on our side — this is a winner. Hold no matter what the book says.
        phase = "settling" if in_settling else "active"
        return False, (
            f"sol_safe_{phase}_dist=${sol_distance:.2f}_"
            f"prob={current_prob:.0%}_peak={st.peak_prob_for_side:.0%}_drop={drop_from_peak:.0%}"
        )

    # === Below here: SOL is NOT safely on our side — bail checks apply ===

    # --- RAPID DROP BAIL: SOL is against us AND prob dropped fast ---
    # Emergency exit: if prob dropped >4% in the last 10s, SOL is actively moving
    # against us. Fire even during settling period — speed matters here.
    if rapid_drop >= DUMP_RAPID_DROP_THRESHOLD:
        log.warning(
            f"[BAIL RAPID DROP] SOL NOT safe (dist=${sol_distance:.2f}) AND "
    ...
    # --- HARD P&L STOP: SOL is against us AND losing big ---
    if st.entry_price_cents is not None:
        exit_price_est = int(current_prob * 100)
        ...
        if unrealized_loss_per_contract >= DUMP_MAX_LOSS_CENTS_PER_CONTRACT:
            log.warning(
                f"[BAIL HARD STOP] SOL NOT safe (dist=${sol_distance:.2f}) AND "
    ...
    # --- REVERSAL BAIL: SOL is against us AND prob has dropped significantly ---
    ...
    if DUMP_ON_PROB_REVERSAL and DUMP_EARLY_EXIT_ENABLED:
        ...
        if drop_from_peak >= effective_threshold:
            ...
            log.warning(
                f"[BAIL REVERSAL] SOL NOT safe (dist=${sol_distance:.2f}) AND "
    ...
    # --- FLOOR: SOL is against us AND prob is at coin-flip ---
    if current_prob < DUMP_PROB_FLIP:
        log.warning(
            f"[BAIL FLOOR] SOL NOT safe (dist=${sol_distance:.2f}) AND "
    ...
    return False, (
        ...
        f"_drop={drop_from_peak:.0%}_rdrop={rapid_drop:.0%}_sol_dist=${sol_distance:.2f}"
    )
...
def compute_qty_from_bankroll(
    ...
    # ABSOLUTE DOLLAR LOSS CAP: cap qty so worst-case loss stays near DUMP_HARD_MAX_LOSS_USD.
    ...
    if cost_per > 0:
        max_qty_for_hard_cap = max(1, int(DUMP_HARD_MAX_LOSS_USD / cost_per))
        if target_qty > max_qty_for_hard_cap:
            log.warning(
                f"[SIZE] HARD DOLLAR CAP: {target_qty} -> {max_qty_for_hard_cap} contracts "
                f"(max loss ${DUMP_HARD_MAX_LOSS_USD:.2f}, entry={entry_cents}¢, "
...
def evaluate_scalp(
    ...
    # Core safety: how far is SOL from the strike?
    ...
    # Volatility sanity check: can SOL actually move `distance` in `secs_to_close`?
...
def main() -> None:
    ...
    log.warning(
        ...
        f"sol_buffer_early=${DUMP_SOL_SAFE_BUFFER_EARLY:.2f} sol_buffer_late=${DUMP_SOL_SAFE_BUFFER_LATE:.2f}"
    )
    log.warning(
    ...
    log.warning("[HEARTBEAT] main() entered — SOL SCALPER is running")
    ...
    def reconcile_on_market_change(new_market: str) -> None:
        ...
        # This prevents seeing other bots' positions and entering HOLD with no entry data.
        pos = pos_tracker.get_signed_qty(new_market)
    ...
    while True:
            ...
            try:
                ...
                if mt2 != st.market:
                    ...
                    if st.traded_this_market and st.entry_price_cents is not None and st.side is not None:
                        ...
                        pos_tracker.record_exit(old_market)
        ...
        # Use INTERNAL position tracker — not the account-wide API feed.
        # This prevents cross-bot interference (seeing other bots' positions).
        pos = pos_tracker.get_signed_qty(st.market)
            ...
            if INDEPENDENT_STOP_LOSS_ENABLED and st.entry_price_cents is not None and st.qty > 0:
                try:
                    ...
                    log.info(
                        ...
                        f"hard_threshold=${DUMP_HARD_MAX_LOSS_USD:.2f} soft_threshold=${current_soft:.2f} | "
                        f"dump_triggered={'YES' if sl_unrealized >= DUMP_HARD_MAX_LOSS_USD else 'no'}"
                    )

                    # HARD STOP: $1.00 — immediate market sell, NO EXCEPTIONS
                    if sl_unrealized >= DUMP_HARD_MAX_LOSS_USD:
                        log.warning(
                            f"[STOP_LOSS TRIGGERED] HARD STOP | {st.market} | {st.side.upper()} | "
                            f"entry={st.entry_price_cents}¢ | bid={sl_our_bid}¢ | "
                            f"qty={st.qty} | loss=${sl_unrealized:.2f} >= ${DUMP_HARD_MAX_LOSS_USD:.2f} | "
                        ...
                        try:
                            sl_payload = build_order_payload(
                                ...
                                count=st.qty,  # Use internal qty, not API
                                post_only=False,
                            )
                            if not DRY_RUN:
                            ...
                            # Reset state
                            pos_tracker.record_exit(st.market)
                            st.sm = SM.DUMPED
                            st.side = None
                            st.entry_price_cents = None
                            st.qty = 0
                ...
                if spot is not None:
                    ...
                    try:
                        ...
                        if should_dump:
                            ...
                            if sell_ok:
                                try:
                                    ...
                                    # Only structural guards above + MAX_CONTRACTS cap below.

                                    if can_flip:
                                        flip_edge = flip_prob - (flip_price / 100.0)

                                        # UNCONDITIONAL flip sizing: always MAX_CONTRACTS
                                        flip_qty = MAX_CONTRACTS

                                        # HARD CAP for flips: reduce qty but always allow 1
                                        flip_cost_per = float(flip_price) / 100.0
                                        flip_worst = flip_cost_per * flip_qty
                                        if flip_worst > DUMP_HARD_MAX_LOSS_USD and flip_qty > 1:
                                            old_fq2 = flip_qty
                                            flip_qty = max(1, min(int(DUMP_HARD_MAX_LOSS_USD / flip_cost_per), MAX_CONTRACTS))
                                            log.warning(
                                                f"[FLIP CAP] {old_fq2} -> {flip_qty} contracts "
                                                f"(worst ${flip_worst:.2f} > ${DUMP_HARD_MAX_LOSS_USD:.2f} cap)"
                                            )
                                    ...
                                    if can_flip:
                                        ...
                                        # Brief pause before placing flip order
                                        time.sleep(3)
                                        ...
                                        if not DRY_RUN:
                                            flip_payload = build_order_payload(
                                                ...
                                                count=int(flip_qty),
                                                post_only=False,
                                                client_order_id=pos_tracker.make_client_order_id(),
                                            )
                                        ...
                                        # No flip — log why (unconditional mode: only structural guards)
                                        reason_parts = []
                        ...
                        if not should_dump and not st.has_scalped and secs_to_close is not None:
                            ...
                            scalp_ask = min(scalp_ask, int(our_prob * 100) - 1)  # -1¢ ensures positive edge
                                ...
                                try:
                                    ...
                                    if scalp_avail is not None and scalp_avail >= MIN_FREE_USD_TO_TRADE:
                                        ...
                                        if scalp_room <= 0:
                                            ...
                                            if scalp_worst > DUMP_HARD_MAX_LOSS_USD and scalp_qty > 1:
                                                old_sq = scalp_qty
                                                scalp_qty = max(1, int(DUMP_HARD_MAX_LOSS_USD / scalp_cost_per))
                                                log.warning(
                                                    f"[SCALP CAP] {old_sq} -> {scalp_qty} contracts "
                                                    f"(worst ${scalp_worst:.2f} > ${DUMP_HARD_MAX_LOSS_USD:.2f} cap)"
                                                )
        ...
        # flip to the wrong side when SOL moves.
        # ================================================================
        ...
        if fast_lane:
            ...
            # --- CROSS-VALIDATE: prob trend must match SOL spot trend ---
            if REQUIRE_TREND_ALIGNMENT and prob_trend_ok:
                ...
                if chosen_side == "yes" and "down" in spot_dir and abs(spot_move) > 1.50:
                    misaligned = True
                    mismatch_reason = f"prob_yes but SOL 60m={spot_dir}(${spot_move:+.2f})"
                elif chosen_side == "no" and "up" in spot_dir and abs(spot_move) > 1.50:
                    misaligned = True
                    mismatch_reason = f"prob_no but SOL 60m={spot_dir}(${spot_move:+.2f})"
                elif chosen_side == "yes" and "down" in spot_short_dir and abs(spot_short_move) > 1.50:
                    misaligned = True
                    mismatch_reason = f"prob_yes but SOL 30m={spot_short_dir}(${spot_short_move:+.2f})"
                elif chosen_side == "no" and "up" in spot_short_dir and abs(spot_short_move) > 1.50:
                    misaligned = True
                    mismatch_reason = f"prob_no but SOL 30m={spot_short_dir}(${spot_short_move:+.2f})"
                ...
                log.info(f"[ALIGNMENT] OK — prob {prob_dir} aligns with SOL 60m={spot_dir} 30m={spot_short_dir}")
        ...
        # 4. CROSS-BOT DRAWDOWN: halve size if shared balance dropped
        ...
        # Reduce qty so worst-case loss stays near DUMP_HARD_MAX_LOSS_USD.
        ...
        if max_possible_loss > DUMP_HARD_MAX_LOSS_USD and qty > 1:
            old_qty = qty
            qty = max(1, min(int(DUMP_HARD_MAX_LOSS_USD / cost_per_contract), MAX_CONTRACTS))
            log.warning(
                f"[PRE-TRADE CAP] max_loss=${max_possible_loss:.2f} > ${DUMP_HARD_MAX_LOSS_USD:.2f}: "
        ...
        if max_risk_usd > 0 and max_possible_loss > max_risk_usd:
            suggested = qty
            qty = max(1, min(int(max_risk_usd / cost_per_contract), MAX_CONTRACTS))
        ...
        use_post_only = POST_ONLY  # Always maker — taker override removed
        ...
        if chosen_side == "yes" and lo is not None:
            ...
            log.warning(
                ...
                f"worst_case_loss=${final_worst_case:.2f} | hard_cap=${DUMP_HARD_MAX_LOSS_USD:.2f} | "
        ...
        try:
            ...
            if fill_status in ("filled", "partial") and filled_qty > 0:
                ...
                if not is_daytime_est():
                    session.overnight_trades_this_hour += 1
                log.warning(f"[FILL] Could not verify fill — assuming filled, using internal tracker")
