...
def run_market_cycle(client: kalshi.KalshiClient, ticker: str,
    ...
    # WO-LANE-FLIP: when KW_MODE=FLIP the flip doctrine runs instead of the directional
    # lanes (which stay intact and dormant). Lazy import avoids an engine<->flip_mode
    # circular import; off by default so the proven engine is unchanged unless opted in.
    from . import flip_mode
    if flip_mode.ENABLED:
        return flip_mode.run_flip_cycle(client, ticker, close_ts, market_obj)
