"""Self-provisioning delta table builder.
...
log = logging.getLogger("k_worker.delta_table_builder")
...
def validate_in_process(csv_file: str, manifest_file: str,
    ...
    from .sessions import SESSION_WINDOWS as SW
    engine_sessions = {s[0] for s in SW} | {"UNKNOWN"}
    ...
    timestamps_arr = [c[0] for c in candles]
    n_candles = len(candles)
    a3_pass = True
...
def build_table(notify_fn=None) -> bool:
    ...
    try:
        ...
        if ok:
            from . import delta_table_loader
            delta_table_loader.load(csv_p)
            tlog("[BUILDER] Hot-loaded into running engine")

            full_transcript = "\n".join(transcript_lines)
            if notify_fn:
                notify_fn(f"<pre>🔧 DELTA TABLE BUILT: {days_covered:.0f}d, "
                          f"{len(candles)} candles, "
                          f"eff_N={len(candles) // 15}, "
                          f"validator PASS\n{validator_transcript}</pre>")
            return True
        else:
            tlog("[BUILDER] Validator FAILED — table NOT loaded")
            if notify_fn:
                notify_fn(f"<pre>🔧 DELTA TABLE BUILD FAILED:\n{validator_transcript}</pre>")
            return False
...
def start_background_build() -> None:
    ...
    def _run():
        build_table(notify_fn=_notify_telegram)
