# bot_xrp.py — Kalshi 15-minute XRP market bot (DATA-DRIVEN V4 Feb 2026)
#
...
# XRP Zone win rates from clean-entry markets
# Note: XRP NO 41-50¢ is a true negative (0% WR) — always blocked
# Zone 21-30¢ has no clean data — blocked
XRP_ZONE_WIN_RATES = [
    (1,  10,  0.895),  # 89.5% WR — 57 markets
    (11, 20,  0.778),  # 77.8% WR — 27 markets
    (21, 30,  0.0),    # No clean data — blocked
    (31, 40,  0.765),  # 76.5% WR — 17 markets
    (41, 50,  0.0),    # 0% WR — true negative, always blocked
    (51, 65,  0.714),  # 71.4% WR — 14 markets
    (66, 80,  0.667),  # 66.7% WR — 3 markets (tiny sample)
    (81, 99,  0.846),  # 84.6% WR — 51 markets
]

XRP_SCORE_THRESHOLD = 2.5

def get_zone_win_rate(book_price: int) -> float:
    for (lo, hi, wr) in XRP_ZONE_WIN_RATES:
        if lo <= book_price <= hi:
            return wr
    return 0.0
...
log = logging.getLogger("kalshi-xrp")
...
def evaluate_poster_entry(p_yes: float, p_no: float,
    ...
    if yes_ask is not None:
        ...
        log.info(
            ...
            f"score={score:.2f} threshold={XRP_SCORE_THRESHOLD}"
        )

        if gap > 0 and score >= XRP_SCORE_THRESHOLD:
            post_price = yes_ask + POSTER_BOOK_PREMIUM
            post_price = max(1, min(99, post_price))
            log.warning(
                f"[TRIGGER-YES] score={score:.2f} >= {XRP_SCORE_THRESHOLD} "
                f"post={post_price}¢"
            )
    ...
    if no_ask is not None:
        ...
        if gap > 0 and score >= XRP_SCORE_THRESHOLD:
            ...
            post_price = max(1, min(99, post_price))
            log.warning(
                f"[TRIGGER-NO] score={score:.2f} >= {XRP_SCORE_THRESHOLD} "
                f"post={post_price}¢"
            )
    ...
    while True:
        ...
        if (now - last_amend) >= POSTER_AMEND_INTERVAL:
            try:
                ...
                if gap <= 0 or score < XRP_SCORE_THRESHOLD:
                    log.warning(
                        f"[GAP-AMEND] Score dropped "
                        f"({score:.2f} < {XRP_SCORE_THRESHOLD}) "
...
def main() -> None:
    ...
    log.warning(f"[IRON RULES] RULE 3: Score >= {XRP_SCORE_THRESHOLD} to post | cost cap ${MAX_COST_PER_MARKET}")
    ...
    log.warning(
        ...
        f"SCORE_THRESHOLD={XRP_SCORE_THRESHOLD} "
            ...
            if entry is None:
                ...
                if (now - last_state_log) >= LOG_STATE_EVERY_SECONDS:
                    ...
                    log.info(
                        ...
                        f"threshold={XRP_SCORE_THRESHOLD} "
...
if __name__ == "__main__":
    try:
        ...
        log.exception(f"FATAL: {e} | {ASSET} V7 SCORE={XRP_SCORE_THRESHOLD} ALLOC=${BOT_ALLOCATION}")
        sys.exit(1)
