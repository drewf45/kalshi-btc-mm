# bot_xrp.py
# Kalshi rolling 15m XRP — Find mispriced contracts, hold to close, scale bankroll
#
# STRATEGY:
# - Arms at T-720s (12 min) to OBSERVE market, XRP price, trends, book
...
# - XRP-AWARE BAIL: only dump if XRP has moved against us, not book noise
...
log = logging.getLogger("kalshi-xrp-bot")
...
# -------------- MULTI-BOT ISOLATION ------------------------------------------------
# All bots share one Kalshi account. Each bot must track its own positions internally
# and only use its share of the available cash.
BOT_ID = "XRP"                # Unique identifier — tags orders and isolates position tracking
NUM_ACTIVE_BOTS = 4           # Total bots on this account (BTC, ETH, SOL, XRP)
...
COINBASE_SPOT_URL = "https://api.coinbase.com/v2/prices/XRP-USD/spot"
...
PROB_MIN = 0.85  # 85%+ to enter in last 5 min — lower bar for better fills, $0.40 cap protects
...
PROB_EARLY_MIN = 0.85            # >8min: need 85%+ — enter earlier for better fills
PROB_MID_ENTRY_SECONDS = 300     # 5-8 min to close = "mid"
PROB_MID_MIN = 0.85              # 5-8min: need 85%+ — enough certainty for fills
...
NO_SIDE_MIN_PROB = 0.80           # NO entries allowed at 80%+ prob (vs 85%+ general)
...
REQUIRE_TREND_ALIGNMENT = True    # Prob trend must match XRP spot trend (borderline only)
# HIGH-CERTAINTY FAST LANE: if prob is this high, skip trend/momentum checks entirely
# Rationale: 90%+ prob means XRP is well inside the range. The outcome is decisive.
...
SPOT_SIGMA_USD_PER_SQRT_SEC = 0.0006  # XRP ~$2.50, ~2x more volatile in % terms; dynamic sigma overrides this
...
# -------------- XRP POSITION SIZE BOOST ----------------------------------------
# XRP is performing well on both sides — increase sizing by 15%.
XRP_SIZE_BOOST_MULT = 1.15
...
COINBASE_CANDLES_URL = "https://api.exchange.coinbase.com/products/XRP-USD/candles"
CANDLES_GRANULARITY_SEC = env_int("CANDLES_GRANULARITY_SEC", 60)
CANDLES_LOOKBACK = env_int("CANDLES_LOOKBACK", 10)
SIGMA_FLOOR = env_float("SIGMA_FLOOR", 0.0003)   # XRP-scaled sigma floor
SIGMA_CEIL = env_float("SIGMA_CEIL", 0.002)      # XRP-scaled sigma ceiling
...
TREND_STRONG_THRESHOLD = 0.003     # $0.003+ move in window = strong trend (XRP-scaled)
TREND_MODERATE_THRESHOLD = 0.0015  # $0.0015+ move = moderate trend (XRP-scaled)
...
# -----------------------------
def fetch_xrp_spot_usd(session: requests.Session, timeout: float = 5.0) -> Optional[float]:
    try:
...
class SessionState:
    ...
    # XRP running P&L tracking (separate from daily_pnl_usd which includes all trades)
    xrp_running_pnl_usd: float = 0.0   # XRP-only cumulative P&L
    xrp_trade_count: int = 0            # Total XRP trades this session
    xrp_wins: int = 0
    xrp_losses: int = 0
    ...
    def record_trade(self, market: str, side: str, entry_price: int, exit_price: Optional[int],
        ...
        if pnl_cents > 0:
            ...
            self.xrp_losses += 1
        log.warning(
            f"[XRP P&L] trade_pnl=${pnl_usd:.2f} running_pnl=${self.xrp_running_pnl_usd:.2f} "
            f"trades={self.xrp_trade_count} W/L={self.xrp_wins}/{self.xrp_losses} "
            f"wr={self.xrp_wins / max(1, self.xrp_trade_count):.0%}"
        )
...
class SpotTrend:
    """
    Tracks XRP spot price over a rolling window to detect trends.
    ...
    def trade_alignment(self, side: str, lo: Optional[float], hi: Optional[float],
        ...
        Logic:
        - YES bet = we think XRP will stay ABOVE lo (or in range)
        - NO bet = we think XRP will stay BELOW hi (or in range)
        - If XRP is trending UP strongly and we want NO → against trend
        - If XRP is trending DOWN strongly and we want YES → against trend
        """
    ...
    def summary(self) -> str:
        ...
        return f"trend={direction}(${move:+.4f}/{mins:.0f}min/{n}pts)"
...
class ProbTrend:
    ...
    def should_buy(self, side: str, current_prob: float, secs_to_close: int = 0) -> Tuple[bool, str]:
        ...
        Time-dependent strictness (more certain early, relax as outcome clarifies):
          EARLY (5-7 min): Require active trend match — XRP still has time to swing.
...
def choose_trade(
    ...
    # probability threshold (80% vs general 85%) when other conditions are met.
...
def _xrp_is_safe(side: str, spot: float, lo: Optional[float], hi: Optional[float],
                  secs_to_close: int) -> Tuple[bool, float]:
    """
    Check if XRP spot price is safely on our side of the market boundary.
    Returns (is_safe, buffer_distance_usd).

    This is THE key check: if XRP is on our side, the book is lying.
    ...
    """
    buffer = DUMP_XRP_SAFE_BUFFER_LATE if secs_to_close < DUMP_XRP_SAFE_CUTOFF_SECONDS else DUMP_XRP_SAFE_BUFFER_EARLY

    if side == "yes" and lo is not None:
        # YES wins if XRP stays ABOVE lo. Safe if spot > lo + buffer.
        distance = spot - lo
        return distance >= buffer, distance
    elif side == "no" and hi is not None:
        # NO wins if XRP stays BELOW hi (range market). Safe if spot < hi - buffer.
        distance = hi - spot
        return distance >= buffer, distance
    elif side == "no" and lo is not None:
        # NO wins if XRP drops BELOW lo (up-or-down market, no hi).
        # Safe if spot < lo - buffer (XRP is well below the strike).
        distance = lo - spot
        return distance >= buffer, distance
...
def should_dump_position(
    ...
    KEY PRINCIPLE: Before any bail trigger fires, check if XRP is on our side
    of the boundary. If it is, the book is lying — HOLD. Only bail when
    XRP has actually moved against us.

    BANKROLL PROTECTION: Never lose more than 5% of balance or 50% of position
    cost on a single trade. This fires before XRP check — no position justifies
    ...
    # For earlier entries, normal dump logic applies — XRP can still move.
    # Was 60s — XRP can move $0.0046 in 60s (σ=0.0006, √60=7.7). 30s is $0.0033, safer.
    ...
    if st.entry_time > 0 and secs_to_close <= HOLD_TO_SETTLE_SECONDS and not is_scalp_entry:
        ...
        if not xrp_safe_for_hold or xrp_hold_dist < HOLD_XRP_DANGER_BUFFER:
            # XRP is near the boundary — DON'T suppress dumps, let normal logic decide
            log.warning(
                f"[HOLD OVERRIDE] XRP near boundary (dist=${xrp_hold_dist:.4f} < ${HOLD_XRP_DANGER_BUFFER:.4f}) "
        ...
        if current_balance_usd > 0:
            ...
            if total_loss_usd > max_loss_balance:
                log.warning(
                    ...
                    f"(entry={st.entry_price_cents}¢ est_exit={exit_price_est}¢) — "
                    f"overrides XRP safety, protecting bankroll"
                )
    ...
    # =============================================================
    # === CATASTROPHIC STOP: absolute backstop, fires BEFORE XRP check ===
    ...
    if st.entry_price_cents is not None:
        ...
        if catastrophic_loss >= DUMP_CATASTROPHIC_LOSS_CENTS:
            log.warning(
                ...
                f"(entry={st.entry_price_cents}¢ est_exit={exit_price_est}¢) — "
                f"overrides XRP safety, capping damage"
            )
            return True, f"catastrophic_{catastrophic_loss}c_per_contract"

    # =============================================================
    # === XRP SAFETY CHECK: THE MASTER OVERRIDE ===
    # If XRP is on our side of the boundary, DO NOT BAIL.
    # The book can spike, the probability can drop on a thin book,
    # but if XRP is $0.003+ on our side with 1 minute left, we WIN.
    # =============================================================
    xrp_safe, xrp_distance = _xrp_is_safe(st.side, spot, lo, hi, secs_to_close)
    if xrp_safe:
        # XRP is on our side — this is a winner. Hold no matter what the book says.
        phase = "settling" if in_settling else "active"
        return False, (
            f"xrp_safe_{phase}_dist=${xrp_distance:.4f}_"
            f"prob={current_prob:.0%}_peak={st.peak_prob_for_side:.0%}_drop={drop_from_peak:.0%}"
        )

    # === Below here: XRP is NOT safely on our side — bail checks apply ===

    # --- RAPID DROP BAIL: XRP is against us AND prob dropped fast ---
    # Emergency exit: if prob dropped >4% in the last 10s, XRP is actively moving
    # against us. Fire even during settling period — speed matters here.
    if rapid_drop >= DUMP_RAPID_DROP_THRESHOLD:
        log.warning(
            f"[BAIL RAPID DROP] XRP NOT safe (dist=${xrp_distance:.4f}) AND "
    ...
    # --- HARD P&L STOP: XRP is against us AND losing big ---
    if st.entry_price_cents is not None:
        exit_price_est = int(current_prob * 100)
        ...
        if unrealized_loss_per_contract >= DUMP_MAX_LOSS_CENTS_PER_CONTRACT:
            log.warning(
                f"[BAIL HARD STOP] XRP NOT safe (dist=${xrp_distance:.4f}) AND "
    ...
    # --- REVERSAL BAIL: XRP is against us AND prob has dropped significantly ---
    ...
    if DUMP_ON_PROB_REVERSAL and DUMP_EARLY_EXIT_ENABLED:
        ...
        if drop_from_peak >= effective_threshold:
            ...
            log.warning(
                f"[BAIL REVERSAL] XRP NOT safe (dist=${xrp_distance:.4f}) AND "
    ...
    # --- FLOOR: XRP is against us AND prob is at coin-flip ---
    if current_prob < DUMP_PROB_FLIP:
        log.warning(
            f"[BAIL FLOOR] XRP NOT safe (dist=${xrp_distance:.4f}) AND "
    ...
    return False, (
        ...
        f"_drop={drop_from_peak:.0%}_rdrop={rapid_drop:.0%}_xrp_dist=${xrp_distance:.4f}"
    )
...
def compute_qty_from_bankroll(
    ...
    # XRP SIZE BOOST: XRP is performing well — multiply all positions by 1.15
    target_qty = int(target_qty * XRP_SIZE_BOOST_MULT)
...
def compute_scalp_qty(
    ...
    if target_qty > 0:
        ...
        log.info(
            ...
            f"(dist=${distance_usd:.4f}, frac={scalp_fraction:.0%}, "
...
def evaluate_scalp(
    ...
    # Core safety: how far is XRP from the strike?
    ...
    # Volatility sanity check: can XRP actually move `distance` in `secs_to_close`?
    ...
    if distance < max_expected_move:
        return False, 0, None, (
            f"vol_unsafe: dist=${distance:.4f} < 1.5σ√t=${max_expected_move:.0f} "
    ...
    reason = (
        f"dist=${distance:.4f} 3σ√t=${max_expected_move:.0f} "
...
def main() -> None:
    ...
    while True:
            ...
            try:
                ...
                if mt2 != st.market:
                    ...
                    if st.traded_this_market and st.entry_price_cents is not None and st.side is not None:
                        # Reconcile st.qty with actual API position before computing P&L.
                        # Resting orders may not have filled (or only partially filled).
                        # API read is OK here — we're checking OUR specific market at settlement.
                        try:
                        ...
                        if should_dump:
                            ...
                            try:
                                ...
                                if not DRY_RUN:
                                    ...
                                    sell_ok = True
                                    position_tracker.record_exit(st.market)
                                else:
                            ...
                            if sell_ok:
                                try:
                                    ...
                                    if can_flip:
                                        ...
                                        if not DRY_RUN:
                                            flip_client_oid = position_tracker.make_client_order_id()
                                            flip_payload = build_order_payload(
                                                ...
                                                count=int(flip_qty),
                                                post_only=False,
                                                client_order_id=flip_client_oid,
                                            )
                                            ...
                                            if flip_fill_status in ("filled", "partial") and flip_filled > 0:
                                                ...
                                                st.has_flipped = True
                                                st.traded_this_market = True
                                                position_tracker.record_fill(st.market, flip_side, flip_filled, int(flip_price), flip_oid)
                            ...
                            if should_scalp:
                                ...
                                try:
                                    ...
                                    if scalp_avail is not None and scalp_avail >= MIN_FREE_USD_TO_TRADE:
                                        ...
                                        if scalp_qty > 0:
                                            scalp_client_oid = position_tracker.make_client_order_id()
                                            scalp_payload = build_order_payload(
                                                ...
                                                client_order_id=scalp_client_oid,
                                            )
                                            ...
                                            if not DRY_RUN:
                                                ...
                                                if scalp_fill_status in ("filled", "partial") and scalp_filled > 0:
                                                    st.has_scalped = True
                                                    position_tracker.add_qty(st.market, scalp_filled)
        ...
        if secs_to_close > BUY_START_SECONDS:
            ...
            if (now - last_state_log) >= LOG_STATE_EVERY_SECONDS:
                ...
                log.info(
                    ...
                    f"60m={trend_dir}(${trend_move:+.4f}) 30m={trend_s_dir}(${trend_s_move:+.4f}) "
        ...
        # flip to the wrong side when XRP moves.
        # ================================================================
        ...
        if fast_lane:
            ...
            if either_strong_against:
                if TREND_AGAINST_BLOCK:
                    log.warning(
                        ...
                        f"60m={trend_dir}(${trend_move:+.4f}) 30m={trend_short_dir}(${trend_short_move:+.4f}). "
            ...
            # --- CROSS-VALIDATE: prob trend must match XRP spot trend ---
            if REQUIRE_TREND_ALIGNMENT and prob_trend_ok:
                ...
                misaligned = False
                if chosen_side == "yes" and "down" in spot_dir and abs(spot_move) > 0.0015:
                    misaligned = True
                    mismatch_reason = f"prob_yes but XRP 60m={spot_dir}(${spot_move:+.4f})"
                elif chosen_side == "no" and "up" in spot_dir and abs(spot_move) > 0.0015:
                    misaligned = True
                    mismatch_reason = f"prob_no but XRP 60m={spot_dir}(${spot_move:+.4f})"
                elif chosen_side == "yes" and "down" in spot_short_dir and abs(spot_short_move) > 0.0015:
                    misaligned = True
                    mismatch_reason = f"prob_yes but XRP 30m={spot_short_dir}(${spot_short_move:+.4f})"
                elif chosen_side == "no" and "up" in spot_short_dir and abs(spot_short_move) > 0.0015:
                    misaligned = True
                    mismatch_reason = f"prob_no but XRP 30m={spot_short_dir}(${spot_short_move:+.4f})"
                ...
                log.info(f"[ALIGNMENT] OK — prob {prob_dir} aligns with XRP 60m={spot_dir} 30m={spot_short_dir}")
        ...
        # Reserve fair share for other bots running on same account
        ...
        entry_client_oid = position_tracker.make_client_order_id()
