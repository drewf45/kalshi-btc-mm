...
def test_swing_to_take_still_fires(flip, gateway, ledger):
    ...
    entry's take now rests at entry + OPEN_GOUGE_C (=64, cap 90) and a fill there
    realizes +4."""
    o = _entry(flip, gateway, ledger, entry=60)
    assert o["take_oid"] == "OID-T1"       # the take rested
    assert o["take_px"] == 64              # entry + OPEN_GOUGE_C, cap 90
    ledger.record_fill(TICKER, "FLIP", "yes", "EXIT", 64, 1, "PROBE")
    flip.note_exit(TICKER, "yes", 64, CLOSE - 700, count=1)
    assert flip.windows[TICKER].window_realized == 4   # 64 − 60
