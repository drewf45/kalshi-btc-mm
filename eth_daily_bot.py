...
print(f"BOOT: eth_daily_bot.py v5 loaded at {datetime.now(timezone.utc).isoformat()}Z", flush=True)

# ======================== LOGGING ============================
LOG_LEVEL = os.getenv("LOG_LEVEL", "INFO").upper()
logging.basicConfig(level=LOG_LEVEL, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("kalshi-eth-daily")
...
# ======================== CONFIGURATION ======================
BOT_ID          = "ETH"
SERIES_TICKER   = getenv_first(["SERIES", "KALSHI_SERIES"], "KXETHD")
...
WATCH_WINDOW_SECONDS = 1200  # 20 minutes before close      # Start watching 90s before close
...
SAFETY_NET_SECONDS   = 120      # Fallback: buy best side at T=10s if no position yet
...
def main() -> None:
    ...
    log.warning("=" * 60)
    log.warning(f"[BOT] ETH DAILY — PARTICIPATION-FIRST V5")
    ...
    while True:
        ...
        if time.time() - last_state_log > 10.0:
            ...
            spot_str = (f"${spot:.8f}".rstrip("0").rstrip(".") if spot < 0.01 else f"${spot:.2f}") if spot else "n/a"
