...
class WindowEcon:
    ...
    # ── §1: the bracket ────────────────────────────────────────────────
    def open_bracket(self, market: str, account_value_cents: int,
                     now: Optional[float] = None) -> None:
        """At the FIRST order submit on this market."""
        ...
        self.ledger.db.execute(
            "INSERT INTO window_econ (ts, market, open_value_cents) VALUES (?,?,?)",
            (now, market, account_value_cents))
        self.ledger.db.commit()
        log.info("WINDOW_ECON open %s at %dc", market, account_value_cents)
    ...
    def close_bracket(self, market: str, account_value_cents: int,
        ...
        self.ledger.db.execute(
            ...
            (account_value_cents, cash_moves, window_pnl, fills_pnl_cents,
             lanes_active, fills_count, market))
        self.ledger.db.commit()
