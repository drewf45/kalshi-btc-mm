...
def boot_tape(recorder=None, boot_caps=None, auth_line=None) -> List[str]:
    ...
    lines.append("  THE DRIFT RECONCILIATION (WO-2026-07-28-X, build 92): X7 — "
                 "the P0. `high_water_cents` fed the owed ratchet off "
                 "`trading_equity_cents` = book_cents (the DERIVED book, the very "
                 "number X1/X2 drift), so a phantom could mint owed. Now "
                 "trading_equity reads the VENUE's cash (WO-V stamp) minus "
                 "externals in LIVE: at a FLAT moment cash IS the realized account "
                 "value (venue-truthful), and mid-window cash is depressed by "
                 "deployed capital — which only ever reads the equity LOW, and the "
                 "high-water is a max(), so a low read never advances it "
                 "(ratchet-safe: owed is never minted on an unrealized mark or a "
                 "phantom). The one-time drift reconciliation prints venue-truth vs "
                 "the old derived book with the delta explained line-item "
                 "(quarantine adjustments, now retired by X5; float dust, now 0 by "
                 "X6; X1 bell-spillover in the residual), tagged RESTATED — owed "
                 "UNCHANGED by the source swap. SHADOW / pre-first-read fall back to "
                 "the paper book, byte-identical. F byte-identical")
    lines.append("  BELL-GROUP BRACKETS (WO-2026-07-28-X, build 92): X5 — the "
                 "account-delta check moved from the market to the BELL (the "
                 "settlement instant). X1's root: differencing a GLOBAL book PER "
                 "MARKET absorbed the OTHER rooms settling on the same bell, so "
                 "the divergence alarm cried wolf on every shared bell (three "
                 "rooms at 21:15). Now per-market attribution IS fills-math (a "
                 "market's P&L is what its fills earned, by definition), and at "
                 f"each bell (members within BELL_GRACE_S={config.BELL_GRACE_S}s) "
                 "reconcile_bell sums the members' fills vs the ONE account delta "
                 "across the bell — a match means the rooms' books are trustworthy, "
                 "a break (a phantom, a missing fill) pages BELL_ECON_DIVERGENCE "
                 "with the members and marks the bell DISPUTED (out of cells), "
                 "touching no headline; a straggler beyond grace forms its own "
                 "bell. The X2 per-market quarantine/RE-BOOK is RETIRED — with "
                 "account truth venue-read (X4) and owed off venue cash (X7), a "
                 "disputed number can inflate nothing, so it is quarantined, never "
                 "overwritten with fills-math (the inversion of Drew's law is "
                 "gone). F byte-identical")
