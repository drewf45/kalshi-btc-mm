...
def size_order(book_cents: int, price_cents: int,
    ...
    if lane == "FLIP":
        ...
        # the retired NET_RISK_CROSS_LANE_CAP no longer binds it.
        # WO-2026-07-24-D Part 1: FLIP self-scales by NOTIONAL (pct of book), NOT
        # Kelly — Kelly capped it at ~5-6 on a $43 book (358c/60c), so
        # FLIP_SIZE_CAP=10 never bit and the +4×10 test ran at half the ruled
        # size. F alone had this bypass; FLIP now shares it (the third instance
        # of a fix on one of two siblings). DEPTH is the term the size test
        # measures; the per-lane at-risk WALL (15%) is the gateway backstop above
        # this. Guard (d): all terms + kelly (n/a) + the binding one logged.
        cap = config.FLIP_SIZE_CAP
        notional_max = int(book_cents * config.FLIP_NOTIONAL_PCT // price_cents)
        contracts = min(notional_max, depth_max, cap)
        term = {"notional": notional_max, "depth": depth_max, "cap": cap}
        bound = min(term, key=term.get)
        return SizeDecision(
            "-", contracts,
            f"FLIP: min(notional={notional_max}, depth={depth_max}, "
            f"cap={cap}, kelly n/a={kelly_max}) → {bound} bound")
