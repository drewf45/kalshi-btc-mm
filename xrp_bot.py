...
# while XRP is $150 on our side is NOT a reason to bail.
...
DUMP_XRP_SAFE_BUFFER_EARLY = 0.02    # >2min to close: need $0.02 buffer (scaled for XRP ~$2.50)
DUMP_XRP_SAFE_BUFFER_LATE = 0.01     # <2min to close: need $0.01 buffer (scaled for XRP ~$2.50)
...
SCALP_DISTANCE_TIERS = [
    (0.10, 0.85),    # $0.10+ from strike: extremely safe, size up hard
    (0.05, 0.65),    # $0.05-0.10: very safe, go bigger
    (0.025, 0.45),   # $0.025-0.05: safe, meaningful size
    (0.015, 0.25),   # $0.015-0.025: moderate — compound the edge
]
...
def should_dump_position(
    ...
    # Was 60s — XRP can move $90 in 60s (σ=12, √60=7.7). 30s is $65, safer.
    ...
    # but if XRP is $100+ on our side with 1 minute left, we WIN.
    # =============================================================
...
def main() -> None:
    ...
    log.warning("[HEARTBEAT] main() entered — XRP SCALPER is running")
