# eth_bot.py — Kalshi 15-minute ETH market bot (DATA-DRIVEN V4 Feb 2026)
#
...
# ETH gap thresholds by book price zone
ETH_GAP_THRESHOLDS = [
    (1,  10,  4),   # 96.1% WR — very strong zone
    (11, 20,  5),   # 87.1% WR — good zone
    (21, 30, 10),   # 66.7% WR — weak, need high conviction
    (31, 40, 10),   # 61.9% WR — weak, need high conviction
    (41, 50, 10),   # 63.6% WR — weak, need high conviction
    (51, 65,  7),   # 71.4% WR — moderate zone
    (66, 80,  7),   # 71.4% WR — moderate zone
    (81, 99,  5),   # 91.2% WR — strong zone
]

def get_min_gap(book_price: int) -> int:
    for (lo, hi, gap) in ETH_GAP_THRESHOLDS:
        if lo <= book_price <= hi:
            return gap
    return 99
...
log = logging.getLogger("kalshi-eth")
