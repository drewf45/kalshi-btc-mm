...
print(f"BOOT: shib_daily_bot.py v5 loaded at {datetime.now(timezone.utc).isoformat()}Z", flush=True)

# ======================== LOGGING ============================
LOG_LEVEL = os.getenv("LOG_LEVEL", "INFO").upper()
logging.basicConfig(level=LOG_LEVEL, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("kalshi-shib-daily")
...
# ======================== CONFIGURATION ======================
BOT_ID          = "SHIB"
SERIES_TICKER   = getenv_first(["SERIES", "KALSHI_SERIES"], "KXSHIBA")
...
def main() -> None:
    ...
    log.warning("=" * 60)
    log.warning(f"[BOT] SHIB DAILY — PARTICIPATION-FIRST V5")
