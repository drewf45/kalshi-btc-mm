# bot.py
# Kalshi rolling 1H BTC — Find mispriced contracts, hold to close, scale bankroll
#
# STRATEGY (adapted from 15-min bot for 1-HOUR markets):
# - Arms at T-2880s (48 min) to OBSERVE market, BTC price, trends, book
...
#
# KEY SETTINGS:
# - TIME-DEPENDENT PROB: 90% if >20min, 87% if 12-20min, 83% if <12min
...
# - Dump abort: 6% reversal, 45s patience, catastrophic 20¢ backstop
...
log = logging.getLogger("kalshi-btc-1h")
...
SERIES_TICKER = getenv_first(["SERIES", "KALSHI_SERIES", "KALSHI_SERIES_TICKER"], "KXBTC1H")
...
PROB_TREND_MIN_SAMPLES = 12       # Need at least 12 samples (~120s at 1/sec)
PROB_TREND_THRESHOLD = 0.06       # 6% swing in one direction = trend signal (more gradual in 1H)
...
# 1H: BTC can move ~2x farther (σ√3600=720 vs σ√900=360), so wider buffers.
DUMP_BTC_SAFE_BUFFER_EARLY = 150.0   # >10min to close: need $150 buffer (σ√600=$294, $150 gives margin)
DUMP_BTC_SAFE_BUFFER_LATE = 80.0     # <10min to close: need $80 buffer (σ√300=$208, $80 is safe)
DUMP_BTC_SAFE_CUTOFF_SECONDS = 600   # 10 min boundary between early/late buffer
...
DUMP_GRACE_PERIOD_SECONDS = 15      # 15s grace period (slightly longer for hourly — more initial noise)
DUMP_PROACTIVE_AFTER_SECONDS = 45   # Proactive bail after 45s (more settling time for hourly markets)
...
DUMP_PEAK_WINDOW_SECONDS = 45  # Use max prob over last 45s as "peak" (wider for hourly noise)
...
FLIP_MIN_TIME_REMAINING = 30        # Need more time for hourly (30s to place + settle)
...
# Over 24 markets/day: extra income if hit rate matches.
SCALP_ENABLED = True
SCALP_MAX_SECONDS = 120            # Only scalp in the last 120 seconds (2 min for hourly — wider window)
SCALP_MIN_SECONDS = 5              # Don't scalp in the last 5s (order might not fill)
SCALP_MIN_DISTANCE_USD = 100.0     # BTC must be ≥$100 from strike (2x for hourly vol)
# Distance tiers: farther from strike = more aggressive sizing
# Each tier: (min_distance_usd, bankroll_fraction)
# 1H: All distances ~2x higher since BTC can move further
SCALP_DISTANCE_TIERS = [
    (800.0, 0.85),   # $800+ from strike: extremely safe, size up hard
    (400.0, 0.65),   # $400-800: very safe, go bigger
    (200.0, 0.45),   # $200-400: safe, meaningful size
    (100.0, 0.25),   # $100-200: moderate — compound the edge
]
...
SIGMA_FLOOR = env_float("SIGMA_FLOOR", 8.0)   # Slightly higher floor for hourly (more sustained vol)
SIGMA_CEIL = env_float("SIGMA_CEIL", 50.0)   # Higher ceiling for hourly (vol spikes matter more)
...
HIGH_CERTAINTY_TIME_SEC = env_int("HIGH_CERTAINTY_TIME_SEC", 30)  # Last 30s for hourly
...
BOUNDARY_BUFFER_USD = env_float("BOUNDARY_BUFFER_USD", 100.0)  # $100 buffer for hourly — BTC can move further
...
# 1H: Longer windows — each market is 60min, so we track across ~2-3 markets
TREND_WINDOW_MINUTES = 120         # Long-term trend: 120 min (~2 markets)
TREND_SHORT_WINDOW_MINUTES = 60    # Short-term trend: 60 min (~1 market)
TREND_SAMPLE_INTERVAL_SECONDS = 30  # Record spot every 30s
TREND_STRONG_THRESHOLD = 150.0     # $150+ move in window = strong trend (wider for hourly)
TREND_MODERATE_THRESHOLD = 75.0    # $75+ move = moderate trend
...
def should_dump_position(
    ...
    HOLD_TO_SETTLE_SECONDS = 60  # Only suppress dumps in the last 60s for hourly (was 30s for 15m)
    HOLD_BTC_DANGER_BUFFER = 150.0  # If BTC is within $150 of boundary, DON'T suppress dumps (2x for hourly)
...
def main() -> None:
    ...
    log.warning("[HEARTBEAT] main() entered — BTC 1H SCALPER is running")
    ...
    while True:
        ...
        taker_prob_thresh = 0.90 if secs_to_close > SETTLEMENT_LOCK_SECONDS else 0.85  # SETTLEMENT_LOCK=600s for 1H
